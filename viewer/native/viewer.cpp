...
#include <filesystem>
namespace sinen {
std::vector<actor> viewer::m_actors;
std::vector<matrix4> viewer::m_matrices;
int viewer::index_actors = 0;
int viewer::index_components = 0;
std::string viewer::current_file_name = "";
class viewer::implements {
public:
  bool is_run = false;
  bool is_save = false;
};
...
void viewer::load_scene(const std::string &path) {
  index_actors = 0;
...
void viewer::save_scene(const std::string &path) {
  std::string str;
  str = "{}";
  json j;
  j.parse(str);
...
viewer::viewer() : m_impl(std::make_unique<viewer::implements>()) {}
viewer::~viewer() {}
void viewer::setup() {
...
void viewer::update(float delta_time) {
  if (request_pop_func) {
