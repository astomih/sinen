...
namespace nen
{
    ...
    bool Animation::Load(const std::string &fileName)
    {
        ...
        if (!file.is_open())
        {
            Logger::Error("File not found: Animation %s", fileName.c_str());
            return false;
        }
        ...
        if (!doc.IsObject())
        {
            Logger::Error("Animation %s is not valid json", fileName.c_str());
            return false;
        }
        ...
        if (ver != 1)
        {
            Logger::Error("Animation %s unknown format", fileName.c_str());
            return false;
        }
        ...
        {
            Logger::Error("Animation %s doesn't have a sequence.", fileName.c_str());
            return false;
        }
        ...
        {
            Logger::Error("Sequence %s has invalid frames, length, or bone count.", fileName.c_str());
            return false;
        }
        ...
        if (!tracks.IsArray())
        {
            Logger::Error("Sequence %s missing a tracks array.", fileName.c_str());
            return false;
        }
        ...
        for (rapidjson::SizeType i = 0; i < tracks.Size(); i++)
        {
            ...
            {
                Logger::Error("Animation %s: Track element %d is invalid.", fileName.c_str(), i);
                return false;
            }
            ...
            {
                Logger::Error("Animation %s: Track element %d is missing transforms.", fileName.c_str(), i);
                return false;
            }
            ...
            {
                Logger::Error("Animation %s: Track element %d has fewer frames than expected.", fileName.c_str(), i);
                return false;
            }
