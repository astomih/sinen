...
void dungeon_generator(sinen::Grid<int> &grid) {
  bool recreate = false;
  {
    ...
    glm::ivec2 minimum_room_size{5, 5};
    glm::ivec2 maximum_room_size{5 + maximum_room_number - room_number,
    ...
    auto stairs = rooms[rooms.size() - 1].get_position();
    ...
    auto keys = rooms[rooms.size() / 2].get_position();
    ...
    auto player_start = rooms[0].get_position();
