...
namespace nen::es
{
	...
	void ESRenderer::SetRenderer(Renderer *renderer)
	{
		mRenderer = renderer;
	}
	...
	void ESRenderer::UpdateVertexArray(const VertexArray &vArray, std::string_view name)
	{
		es::VertexArrayForES vArrayGL;
		vArrayGL.indexCount = vArray.indexCount;
		vArrayGL.indices = vArray.indices;
	...
	{
		ShaderES pipeline;
		pipeline.Load(std::string("Assets/Shader/GLES/") + shaderInfo.vertName + std::string(".vert"), std::string("Assets/Shader/GLES/") + shaderInfo.fragName + std::string(".frag"));
	...
	bool ESRenderer::loadShader()
	{
		...
		if (!mSpriteShader->Load("Assets/Shader/GLES/shader.vert", "Assets/Shader/GLES/shader.frag"))
		{
			return false;
		}
		ShaderParameters param{};
		...
		if (!mAlphaShader->Load("Assets/Shader/GLES/shader.vert", "Assets/Shader/GLES/alpha.frag"))
		{
			return false;
		}
