...
constexpr Vector3f Vector3f::Zero(0.0f, 0.0f, 0.f);
constexpr Vector3f Vector3f::UnitX(1.0f, 0.0f, 0.0f);
constexpr Vector3f Vector3f::UnitY(0.0f, 1.0f, 0.0f);
constexpr Vector3f Vector3f::UnitZ(0.0f, 0.0f, 1.0f);
constexpr Vector3f Vector3f::NegUnitX(-1.0f, 0.0f, 0.0f);
constexpr Vector3f Vector3f::NegUnitY(0.0f, -1.0f, 0.0f);
constexpr Vector3f Vector3f::NegUnitZ(0.0f, 0.0f, -1.0f);
constexpr Vector3f Vector3f::Infinity(Math::Infinity, Math::Infinity, Math::Infinity);
constexpr Vector3f Vector3f::NegInfinity(Math::NegInfinity, Math::NegInfinity, Math::NegInfinity);
...
Vector2f Vector2f::Transform(const Vector2f& vec, const Matrix3& mat, float w /*= 1.0f*/)
{
	Vector2f retVal;
...
Vector3f Vector3f::Transform(const Vector3f& vec, const Matrix4& mat, float w /*= 1.0f*/)
{
	Vector3f retVal;
...
Vector3f Vector3f::TransformWithPerspDiv(const Vector3f& vec, const Matrix4& mat, float w /*= 1.0f*/)
{
	Vector3f retVal;
	retVal.x = vec.x * mat.mat[0][0] + vec.y * mat.mat[1][0] +
		vec.z * mat.mat[2][0] + w * mat.mat[3][0];
...
Vector3f Vector3f::Transform(const Vector3f& v, const Quaternion& q)
{
	// v + 2.0*cross(q.xyz, cross(q.xyz,v) + q.w*v);
	Vector3f qv(q.x, q.y, q.z);
	Vector3f retVal = v;
	retVal += 2.0f * Vector3f::Cross(qv, Vector3f::Cross(qv, v) + q.w * v);
	return retVal;
}
/*
Effekseer::Vector3D Vector3f::ToEffekseer() const
{
...
/*
Effekseer::Matrix44 Matrix4::ToEffekseer() const
{
	...
	for (int y = 0; y < 4; y++)
	{
		for (int x = 0; x < 4; x++)
		{
			mat.Values[y][x] = this->GetAsFloatPtr()[x + y * 4];
		}
	}
	return mat;
}
