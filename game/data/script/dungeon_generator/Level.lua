local FuncModule = require("dungeon_generator/helpFunctions")
local TileModule = require("dungeon_generator/Tile")
local RoomModule = require("dungeon_generator/Room")
...
function Level:setMaxRoomSize(m)
    if m > min(self.height, self.width) or m < 3 then
        error(
            "MaxRoomSize can't be bigger than height-3/width-3 or smaller than 3")
    end
    self.maxRoomSize = m
end
