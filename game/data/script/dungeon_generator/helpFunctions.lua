...
function getAdjacentPos(row, col)
    ...
    local result = {}
    for dx = -1, 1 do
        for dy = -1, 1 do result[#result + 1] = {row + dy, col + dx} end
    end
    for i = 1, #result do end
    return result
end
...
function findNext(start, goal)
    ...
    for i = 1, #adj do
        local adjT = adj[i]
        if (getDist(adjT, goal) < dist) and i % 2 == 0 -- not picking diagonals (would cause too narrow corridors)
        then
            nextPos = adjT
...
function table.clone(org)
    local newTable = {}
    for k, v in pairs(org) do newTable[k] = v end
    return newTable
end
...
function getDist(start, goal)
    return math.sqrt(math.pow(math.abs(goal[1] - start[1]), 2) +
