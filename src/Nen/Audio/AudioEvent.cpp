...
namespace nen
{
	AudioEvent::AudioEvent(std::shared_ptr<AudioSystem> audiosystem, std::string_view name, uint32_t sourceID)
	...
	void AudioEvent::SetPosition(Vector3 pos)
	{
		this->pos = pos;
	...
	const Vector3 &AudioEvent::GetPosition()
	{
		return pos;
	}
