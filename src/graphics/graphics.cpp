...
void Graphics::set_clear_color(const Color &color) {
  GraphicsSystem::set_clear_color(color);
}
Color Graphics::clear_color() { return GraphicsSystem::get_clear_color(); }
...
void Graphics::bind_pipeline3d(const GraphicsPipeline3D &pipeline) {
  GraphicsSystem::bind_pipeline3d(pipeline);
}
void Graphics::bind_default_pipeline3d() {
  GraphicsSystem::bind_default_pipeline3d();
}
void Graphics::bind_pipeline2d(const GraphicsPipeline2D &pipeline) {
  GraphicsSystem::bind_pipeline2d(pipeline);
}
void Graphics::bind_default_pipeline2d() {
  GraphicsSystem::bind_default_pipeline2d();
}
void Graphics::set_uniform_data(uint32_t slot, const UniformData &data) {
  GraphicsSystem::set_uniform_data(slot, data);
}
void Graphics::begin_target2d(const RenderTexture &texture) {
  GraphicsSystem::begin_target2d(texture);
}
void Graphics::begin_target3d(const RenderTexture &texture) {
  GraphicsSystem::begin_target3d(texture);
}
void Graphics::end_target(const RenderTexture &texture, Texture &out) {
